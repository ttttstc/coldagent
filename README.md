...
![Preview](preview.png)
  ...
  - A number of modules still contain restoration-time fallbacks, so behavior may differ from the original ColdAgent implementation.
  ...
  - bundled skill content for `ColdAgent-api` and `verify` has been rewritten from placeholder files into usable reference docs
  ...
  - 仍有部分模块保留恢复期 fallback，因此行为可能与原始 ColdAgent 实现不同。
  ...
  - `ColdAgent-api` 和 `verify` 的 bundled skill 内容已经从占位文件恢复为可用参考文档
